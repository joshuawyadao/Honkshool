...
| D-023 | Use Kokoro-82M v1.0 voice `bm_george` at model speed `0.86` as the preferred narration and cadence reference. Validate a local iPhone implementation before production integration; do not silently substitute an Apple voice when the accepted output is unavailable. | The owner found both evaluated Kokoro voices markedly more natural than the Apple voices, preferred George's calm documentary delivery, and selected the more spacious cadence. A bounded device check preserves the no-paid-API and local-first goals without assuming an untested runtime. | Accepted 2026-09-21; supersedes D-001 as the production narration direction while preserving its feasibility evidence |
| D-024 | Generate Kokoro narration during controlled content preparation and bundle the verified audio for the curated first prototype. Play it locally with AVFoundation; do not ship the model or a live inference dependency in this slice. | One lossless prepared session is 34,926,044 bytes and has an exact 727.625-second duration, while the pinned model alone is 327,212,226 bytes before runtime dependencies. Prepared audio reproduces the accepted sound, removes synthesis latency and power risk during a nap, and makes the route duration known before approval. | Accepted 2026-09-21; implements D-023 for the curated prototype, subject to physical-iPhone playback and complete-session listening checks |
| D-007 | Which voice, locale, pacing, and pronunciation mechanism should be standardized? Resolved by D-023: Kokoro George at model speed `0.86` is the preferred reference. | Preserve the original question and the complete Apple audition history while separating listening acceptance from remaining iPhone/runtime validation. | Resolved by D-023 on 2026-09-21 |

## Pending decisions

Resolved on 2026-09-15 from the corrected device runs: D-001, D-002, D-003, and D-005. Their accepted decisions and evidence are recorded in the closeout section below. The owner subsequently resolved D-004 and D-009 for Phase 1, then D-007 through D-023 after narration auditions; the dated history below preserves the earlier questions and evidence.
...
- **D-007 — Source voice remains unresolved:** The owner reports persistent robotic texture after I. Unprocessed Aaron samples at rates 0.45 and 0.50 both remain noticeably robotic on iPhone, with the faster one slightly better. The faster WAV also sounded identical in Codex's Mac preview and QuickTime. This supports comparing source voices and makes a Codex-specific or Mac-only defect less likely; it does not establish the exact synthesis defect or rule out every playback contribution. Preserve naturalness over slower syllables and stop escalating consonant attenuation as the default next step.
...
- **Download resolution and accent evidence:** The owner manually downloaded Ava and Jamie Premium. Both resolved as API quality 3; the unchanged passage rendered successfully at rate 0.50. Padded durations are 24.523719 seconds for Ava and 24.176689 for Jamie. Exact PCM/hash checks pass without clipping. These are new listening candidates, not final voice acceptance; Jamie's API identifier contains `Malcolm` despite its display name.
...
## Kokoro narration direction, 2026-09-21

- **D-007/D-023 — Accepted voice and cadence reference:** The owner found Kokoro Heart and George substantially more human and natural than the Apple auditions, preferred George's calm documentary character, and then selected the more spacious George version at model speed `0.86`. Use Kokoro-82M v1.0 voice `bm_george` at speed `0.86` for future narration preparation and evaluation. Heart remains a liked alternative rather than the default, and the earlier Apple work remains historical evidence.
- **Controlled evidence:** The unchanged 74-word closing paragraph at George speed `1.0` measures 25.100 seconds with SHA-256 `4d4a2ce2c3fea0e37a12bbb3b4af97d26cda0c5446d71861d131939d74f91f50`. A 408-word excerpt using the first three catalog paragraphs measures 141.000 seconds at speed `1.0`. The same excerpt measures 148.050 seconds at speed `0.92` and 155.975 seconds at the selected speed `0.86`; the selected WAV has SHA-256 `fa62df1623cf17f6261127cc1d51cbcb055f9fc9e01402b938e14ff2416cafa7`. Model speed controls predicted durations before synthesis; the WAV was not time-stretched. Exact source text, model revision, asset hashes, PCM construction, levels, and absence of clipping were verified locally. Listening establishes preference, not spoken-word accuracy or full-session acceptance.
- **Runtime consequence at acceptance:** D-001 remains valuable evidence that direct AVSpeechSynthesizer playback, background audio, interruptions, and Lock Screen controls work on the target iPhone, but it no longer defines the production narration sound. This checkpoint proposed a bounded on-device Kokoro feasibility slice before adding an inference dependency or replacing the playback adapter. D-024 subsequently chose prepared playback for the curated prototype after comparing the model and rendered-asset costs. No paid or hosted speech service is selected. Do not fall back silently to an Apple voice if the accepted output is unavailable.
- **Preparation boundary at acceptance:** Upstream Kokoro inference code and the v1.0 model are published under Apache 2.0 and run locally without a metered speech fee, subject to the license's notice obligations and a review of every shipped implementation component. Generated auditions and scratch environments remained outside Git at this checkpoint. D-024 records the later bundled narration asset and catalog-estimate change; voice selection and rain remained unchanged.

## Prepared Kokoro playback, 2026-09-21

- **D-024 — Preparation-time architecture:** The complete 1,829-word revision-1 session now renders with the pinned Kokoro model and George `0.86` setting during controlled content preparation. The app bundles the resulting mono 24 kHz PCM WAV and compact provenance, not the model, voice embedding, Python environment, or a live inference runtime. This remains fully local during a nap and adds no paid or hosted service.
- **Measured evidence:** The prepared file measures 727.625 seconds and 34,926,044 bytes, with SHA-256 `7117b18ce10e45844b6eba29936370131290baf30131b71cf2b01d5999847f37`, −22.882915 dBFS RMS, −4.871702 dBFS peak, and no clipping. Its 17,463,000 frames contain the exact versioned catalog paragraphs, one-second paragraph gaps, and 0.25-second end padding. The current configurable estimate is 730 seconds, rounded up to five seconds. The 327,212,226-byte pinned model remains outside the app.
- **Runtime behavior:** Catalog metadata resolves the exact resource and measured duration without importing AVFoundation into the planning domain. The feasibility console uses AVAudioPlayer under the existing exclusive audio-session, interruption, route-change, remote-command, and fallback controller. Missing or failed prepared audio produces a visible failure instead of an Apple-voice substitution. The captured deadline stops prepared narration or subsequent ambience at the planned wake time; it never changes playback speed or alarm time.
- **Evidence boundary:** Automated checks verify asset identity, construction, catalog binding, loading, pause/resume, completion, stale callbacks, invalid deadlines, and fixed-deadline stopping. Generic device compilation and simulator tests are development evidence. A target-iPhone install must still confirm background/locked playback, interruptions, route loss, system controls, cutoff timing, and audio parity, and the owner must hear the complete session before full-session acceptance. The feasibility console does not persist partial playback or completion; that remains production runtime/history work.
- **Follow-up, 2026-09-23:** The prepared build installed on the target iPhone, and the owner confirmed locked playback and Lock Screen play/pause. New simulator checks read every bundled George frame through AVFoundation and use a real AVAudioPlayer near the end of the file to observe natural completion into ambience and a fixed injected cutoff. This reduces the active device checklist to brief route/control and real-alarm delivery checks. A short unchanged review reel covers representative voice listening. These checks do not establish simultaneous real-alarm/narration cutoff or complete-session subjective comfort; those remain unaccepted under D-024 and can be observed during natural use.
...
- A user-facing voice selector remains deferred after selection of George `0.86`; any later alternative needs explicit pre-plan preview, voice-specific timing, and settings fixed for the run.
