...
- **Runtime consequence:** D-001 remains valuable evidence that direct AVSpeechSynthesizer playback, background audio, interruptions, and Lock Screen controls work on the target iPhone, but it no longer defines the production narration sound. Before adding an app dependency or replacing the playback adapter, run a bounded on-device Kokoro feasibility slice covering license notices and transitive components, model/app size, memory, thermals, battery, synthesis latency, audio parity, locked-screen/background behavior, interruption recovery, and fixed-deadline stopping. No paid or hosted speech service is selected. Do not fall back silently to an Apple voice if the accepted model is unavailable.
- **Preparation boundary:** Upstream Kokoro inference code and the v1.0 model are published under Apache 2.0 and run locally without a metered speech fee, subject to the license's notice obligations and a review of every shipped implementation component. Generated auditions and scratch environments remain outside Git. No dependency, model, voice asset, generated narration, production runtime, catalog estimate, selector, or rain mix changes in this documentation checkpoint.
