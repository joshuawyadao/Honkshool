...
The pre-play portion from PR #5 lets the listener choose locally playable prepared content and a duration or exact wake time, review the entire fixed route and fallback, request an alarm, and confirm an in-memory snapshot. Sessions without resolvable bundled narration are excluded from the picker and reviewed route. Review shows a planned start and fixed wake deadline to the second; duration is measured from a start one minute ahead, while short exact wake windows receive a shorter review interval. Confirmation after the planned start refreshes the route and deadline for another explicit approval, and a past exact wake time cannot be confirmed. The bundled catalog currently has one available session and no accepted ambience, so silence is the current rest sound. The production run controller consumes the confirmed snapshot, waits for its approved start, plays the exact prepared route with pause/resume/stop and Lock Screen controls, transitions to silence, and stops audio at the fixed deadline. When requested, a separate production AlarmKit service must authorize, schedule, and verify the one-shot alarm for that same deadline before the run begins. It retains the alarm identity across relaunches, exposes status and explicit cancellation, and requires cancellation of an earlier tracked production alarm before another plan starts. Playback Stop leaves the wake alarm intact. The app must remain foregrounded until narration begins; leaving early ends the pending run. The local-history adapter saves actual completion/partial records and verified checkpoints across relaunch; a delayed cutoff records the actual stop and an earlier verified position without inventing an on-time stop. Recovered checkpoints remain explicitly partial evidence at their captured time. History offers Continue, Resume, and Replay through a fresh review; reopening never restarts playback. An opt-in target-iPhone test passed real alarm alerting and production audio cutoff; locked-screen controls and rain remain future acceptance and feature slices. Current validation is recorded in this branch's task plan.
...
4. Start the plan, keep the app open until narration begins, then lock the phone and listen without in-app attention prompts.
