...
- [ ] Build `feature/alarm-and-history` to schedule and manage only Honkshool alarms before an alarm-requested run starts, persist partial and completed playback locally with SwiftData, advance sessions only on completion, and expose resume and history views.
