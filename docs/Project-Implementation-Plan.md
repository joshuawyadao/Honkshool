...
F is the provisional [narration reference](Narration-Reference.md), accepted through MacBook speakers. Natural delivery takes priority over slowing syllables. On 2026-09-17 the full Mac audition measured 674.222 seconds; its planning estimate is now 675 seconds. A CC0 rain candidate is bundled with provenance, reproducible preparation, and objective loop/level checks. See [Audio-Preparation.md](Audio-Preparation.md). The owner approved the presented tempo/cadence and heard improvement from G/H, but I's additional attenuation did not resolve robotic texture. On 2026-09-20, unprocessed Aaron rate comparisons retained the problem during iPhone WAV playback; faster delivery helped slightly, and the faster file sounded the same in Codex and QuickTime on Mac. A bounded same-paragraph comparison now uses distinct installed Samantha and Daniel voices. Their API quality is default, so this is diagnostic and not an assumed upgrade. See the [source-voice evidence](Narration-Reference.md#source-voice-investigation-2026-09-20).

No short follow-up has replaced the full-session master. Final headphone and full-session acceptance, technical pronunciation, and target-iPhone voice availability/direct-speech equivalence remain open; playing Mac WAVs on iPhone does not close those synthesis checks. Any accepted replacement voice needs a fresh full-session duration measurement. Rain acceptance and its runtime connection are also pending; silence remains available. Phase 2 exit criteria are therefore not yet complete.
