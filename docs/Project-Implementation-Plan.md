...
F is the provisional [narration reference](Narration-Reference.md), accepted through MacBook speakers. Natural delivery takes priority over slowing syllables. On 2026-09-17 the full Mac audition measured 674.222 seconds; its planning estimate is now 675 seconds. A CC0 rain candidate is bundled with provenance, reproducible preparation, and objective loop/level checks. See [Audio-Preparation.md](Audio-Preparation.md). The owner approved the presented tempo and cadence but reported robotic “s” syllables; a short level-only candidate G preserves timing while testing consonant texture. Its listening acceptance remains pending. AirPods/target-iPhone listening, matching the processed Mac audition on the actual playback path, technical pronunciation, and rain acceptance remain open. Rain has no runtime connection yet; silence remains available. Phase 2 exit criteria are therefore not yet complete.
