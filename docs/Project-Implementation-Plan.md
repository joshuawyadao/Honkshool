...
- [x] Build `codex/nap-playback-runtime` to execute alarm-free approved plans without mid-nap prompts, support prepared background narration and media controls, transition to silence, and pause for expected audio interruptions. Audio-position checkpoints and completion evidence remain in memory until the history slice. Target-iPhone runtime acceptance is still pending.
...
- [ ] Build `feature/local-history` to persist partial and completed playback locally with SwiftData, advance sessions only on completion, and expose resume and history views.
