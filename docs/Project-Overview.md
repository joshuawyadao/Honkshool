...
The prepared-content catalog now contains one original citation-backed automotive session, immutable metadata, pronunciation guidance, and a configurable estimate informed by a complete Mac development render. The loader connects it to the planner and validates revision-specific script resume positions. F is accepted as a provisional listening reference from MacBook-speaker listening; this does not establish iPhone voice availability or identical direct-speech output.

Full-session Mac narration now measures 674.222 seconds, with a 675-second planning estimate. A softened CC0 window-rain candidate is bundled with provenance and loop/level checks. See [Audio-Preparation.md](Audio-Preparation.md). Next is listening review and target-iPhone voice/timing verification. D-007 still needs device/headphone and pronunciation evidence; D-008 still needs candidate acceptance. Silence remains available; rain is not yet connected to the planner or runtime. Plan-review UI, runtime cutoff enforcement, and SwiftData history follow; this branch changes no playback or alarm behavior.
