...
The prepared-content catalog now contains one original citation-backed automotive session, immutable metadata, pronunciation guidance, and a configurable estimate informed by a complete historical Mac development render. The loader connects it to the planner and validates revision-specific script resume positions. Kokoro George at model speed `0.86` is the preferred narration and cadence reference after short and 408-word listening comparisons; this does not yet establish a production iPhone runtime.

The historical Aaron full-session render measures 674.222 seconds, with a 675-second planning estimate. A softened CC0 window-rain candidate is bundled with provenance and loop/level checks. See [Audio-Preparation.md](Audio-Preparation.md). Next is a bounded target-iPhone Kokoro feasibility check followed by complete-session George rendering, listening, and timing. D-007's narration direction is resolved through D-023; D-008 still needs rain acceptance. Silence remains available, and rain is not yet connected to the planner or runtime. Plan-review UI, runtime cutoff enforcement, and SwiftData history follow; this documentation checkpoint changes no playback or alarm behavior.
