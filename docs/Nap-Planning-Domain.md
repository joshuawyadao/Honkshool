...
Partial outcomes record stopped, interrupted, or deadline-reached playback. A partial event at the deadline is classified as deadline-reached. A terminal partial record closes that run; its checkpoint can start a later reviewed plan. Pausing and resuming within an ongoing nap belongs to future runtime bookkeeping and must not append a terminal record prematurely. `PreparedSession.narration(resumingAt:)` now validates identity/revision and requires an intact Swift Character boundary before the end of the exact bundled script. The production adapter still owns speech callback mapping, remaining-duration estimation, and valid continuation behavior; no playback engine is added by the catalog.
