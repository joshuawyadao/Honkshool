...
3. Added a [bundled content catalog](docs/Content-Catalog.md) with an original citation-backed automotive session, source metadata, pronunciation guidance, and a configurable estimate informed by a full Mac render. Kokoro George at model speed `0.86` is now the preferred calm-documentary narration reference after short and 408-word listening comparisons. [Audio preparation](docs/Audio-Preparation.md) records the evidence and a bundled CC0 rain candidate; full-session timing, on-device Kokoro feasibility, rain acceptance, and runtime integration remain open.
