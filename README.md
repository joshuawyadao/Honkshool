...
> **Project status:** Feasibility spike plus a pre-play Nap Plan review screen; there is no supported release. The experimental console still measures narration, background audio, and AlarmKit behavior. A separate screen now uses the tested planning domain and bundled prepared-content catalog to choose, review, and confirm a fixed plan. Confirmation does not start playback, schedule an alarm, or save history.
...
3. Added a [bundled content catalog](docs/Content-Catalog.md) with an original citation-backed automotive session, source metadata, pronunciation guidance, and a complete prepared Kokoro George `0.86` narration. The lossless 727.625-second asset now plays through the feasibility console with pause/resume, interruption handling, Lock Screen metadata, and fixed-deadline stopping; the planner uses a configurable 730-second estimate. [Audio preparation](docs/Audio-Preparation.md) records exact provenance. Physical-iPhone playback checks, full-session listening acceptance, rain acceptance, and production plan/history integration remain open.
4. The pre-play choose → review slice is implemented on `codex/nap-plan-review`. Connect the approved snapshot to production playback, alarm scheduling, and history in later branches.
