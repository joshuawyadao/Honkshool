...
3. Added a [bundled content catalog](docs/Content-Catalog.md) with an original citation-backed automotive session, source metadata, pronunciation guidance, and a configurable estimate informed by a full Mac render. Lee/Karen Premium are the preferred short-sample voice direction; accent comparison and final selection remain open in the [narration reference](docs/Narration-Reference.md). [Audio preparation](docs/Audio-Preparation.md) now supplies a measured local full-session audition and a bundled CC0 rain candidate; listening acceptance, iPhone equivalence, and runtime integration remain open.
